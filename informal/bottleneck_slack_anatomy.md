...
**Why huge slack yet no formal route (the paradox resolved):** the slack is *real* but *global*. It is manufactured by the eigenvector equation forcing `g_e²` small exactly on the high-`t_e` (high-overlap) edges — an anti-correlation between the triangle weight `t_e` and the Fiedler drop `g_e`. Term-wise / combinatorial relaxations (`t_e ≤ min(d_a,d_b)−1` → `B2′`; uniform `max_e t_e ≤ degQuad`) cannot see this anti-correlation: they bound each edge by its *worst-case* `t_e` and lose the smallness of `g_e`, so they overshoot wildly (`B2′/2λdegQuad > 1` on sparse-core deg2+dense, `informal/conjecture_B_signed_cancellation.md`). The slack cannot be localized into a per-edge or per-class certificate — and, per the repo's prior negative results, it also cannot be captured by any *fixed* quadratic form. The candidate signed-SOS form `M_C + L` (testing `C = ½(A+I) ≥ −λ`, `M_C[h,h]+=δ`, `M_C[h,l]+=−δ/2`, `δ=d_h−d_l`) is **indefinite** (min eigenvalue −0.13 on `deg2d80_0.1`, `informal/conjecture_B_signed_cancellation.md`), and natural Rayleigh witnesses `φ=M_C f` *overshoot* the vanishing gap by `O(n³)` (`informal/conjecture_B_variational_core.md`). So the eigenvector equation `Lu=λu` is essential in a graph-dependent way that **no** closed quadratic-form certificate reproduces — which is exactly why `aggregate_triangle_poincare` 'must be proved directly' (`ConjectureB.lean` docstring, line 852). There is no sub-additive decomposition to exploit. *(Correction: an earlier draft of this note called `M_C+L` a route that "can recover" the slack — it is in fact a ruled-out route; see `informal/M_C_route_clarification.md`.)*
